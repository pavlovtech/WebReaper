...
- **CONTEXT.md** gains a "Flagged ambiguities" bullet recording the
  decision.
- **A C# 15 `union`-types migration is the natural successor — a
  deferred follow-up (the user endorsed it, 2026-05-22).** C# through
  version 14 does not compiler-check a closed hierarchy's `switch` for
  exhaustiveness — `PageAction`'s `abstract record` + private-ctor +
  sealed-nested-arms shape is the pre-`union` idiom, and the transport's
  `switch` carries a `default: throw` because a forgotten arm is a
  runtime miss, not a compile error (verified: a `switch` *expression*
  over the six arms still raises CS8509 — the compiler does not treat
  the closed hierarchy as exhaustive). C# 15's `union` types (with
  .NET 11, targeted ~November 2026; in preview now) close exactly that
  gap — the compiler *guarantees* a `switch` over a `union` is
  exhaustive, so a forgotten arm becomes a hard compile error. When
  C# 15 is available, `PageAction` should migrate to a `union`; the
  transport's existing `switch` then becomes natively, statically
  exhaustive — the structural enforcement considered option (b)'s
  `Match` would hand-roll, delivered by the language rather than shipped
  as scaffolding to later unwind. Not actioned now: the project targets
  `LangVersion 12`, and a preview language feature does not belong in a
  published library.
