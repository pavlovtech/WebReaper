...
- ADR-0054: `WebReaper.Stealth.CloakBrowser`'s `WithCloakBrowser()` runs install + launch synchronously at builder time (sync-over-async at the boundary). First-use download is ~220 MB from CloakHQ's GitHub releases. Set `CloakBrowserOptions.AutoInstall = AutoInstallPolicy.Disabled` to require pre-installed binaries (CI / airgapped scenarios). The launched process is owned by the OS until WebReaper exits — v10 lacks an engine-teardown disposal hook for builder-time-spawned processes; v10.x will add one as `IAsyncDisposable` reaches into the builder.
