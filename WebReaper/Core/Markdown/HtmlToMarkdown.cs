...
/// <summary>
/// Pure HTML-to-Markdown conversion — the canonical no-schema primitive
/// (ADR-0063). Used by:
/// <list type="bullet">
/// <item><see cref="WebReaper.Core.Parser.Concrete.MarkdownContentExtractor"/>
/// — the thin <c>IContentExtractor</c> adapter shell.</item>
/// <item>The LLM extractor's pre-clean step (ADR-0044).</item>
/// <item>The LLM repairer's pre-clean step (ADR-0047).</item>
/// <item>The change-tracking processor's hash input (ADR-0048).</item>
/// <item>The agent engine's state-building (ADR-0051).</item>
/// </list>
/// <para>
/// The tag-based Readability heuristic (<c>&lt;article&gt;</c> →
/// <c>&lt;main&gt;</c> → <c>[role=main]</c> → <c>&lt;body&gt;</c>) and the GFM
/// rendering are the ADR-0040 grammar — unchanged. Synchronous, AOT-clean,
/// no reflection, no LLM dependency.
/// </para>
/// </summary>
public static class HtmlToMarkdown
{
    ...
    public static MainContent ExtractMainContent(string html)
    {
        ...
        // AngleSharp's HtmlParser.ParseDocument is the synchronous path
        // — no BrowsingContext loader configuration, no Task wrapping.
        // Faster than the OpenAsync path for the no-loader case and
        // keeps the primitive synchronous (ADR-0063 fork — the function
        // is string → MainContent, no async work to wait on).
        ...
        if (root is null)
        {
            // Malformed document — no body, no document element. Return
            // empty (not null, not error). The empty-body case is pinned
            // by the tests.
            return new MainContent(headTitle, string.Empty);
        }

        StripNonContent(root);

        // Title preference: the first surviving <h1> inside main content,
        // else <head><title>. Reading h1 post-strip prevents a
        // "Skip to content" hidden h1 in stripped chrome from winning.
        ...
        return new MainContent(title, markdown);
    }
